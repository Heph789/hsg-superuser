...
- allow the DAO to clawback funds if need be, or execute any transaction on the safe's behalf
...
[HSGSuperMod.sol](https://github.com/Heph789/hsg-superuser/blob/dev/src/HSGSuperMod.sol) is a **Zodiac module** that...
...
4. Allows the owner hat wearer to execute transactions on behalf of the safe
...
3. Execute transactions on the safe's behalf

> Note
> Do NOT add a module on the safe's behalf as the owner. The only module allowed allowed is the HatsSignerGate module.
...
**HSGSuperFactory:** `0x89F804D4Bf5A49966423cBCd259288F24d41d447`

**HSGSuperMod Singleton:** `0x931dBA970D0210C60C11Dbde633ae45d6718F1cc`

**Hats Contract:** `0x3bc1A0Ad72417f2d411118085256fC53CBdDd137`

### Sepolia

**HSGSuperFactory:** `0x85b7B1afb4f8955CC82e6b3fa5d1066dfb2487fA`

**HSGSuperMod Singleton:** `0xff3660d13BfA194744030577296236fA0cd18c70`
...
2. Make sure the parameters for your chain id are set in `script/DeployParams.json`
3. run the script in `script/DeployHatsFactMod.s.sol` (recommended to run the simulation first, then the deploy. make sure you have enough gas!)

### Deploy a HSGSuperMod-enabled Safe

> Presumably, most users are deploying these contracts from their DAO, which already wears its own top hat. For this example, see _script/ProofOfConcept/DeployHatsFromGov.s.sol_
