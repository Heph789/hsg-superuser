...
import { MockGuard } from "./MockGuard.sol";

contract HSGSuperModTest is HSGSMTestSetup {
    // owner can set working guard
    function testAuthorityCanSetGuard() public {
        mockIsWearerCall(address(this), ownerHat, true);
        MockGuard guard = new MockGuard();
        hsgsuper.changeGuard(address(guard));
        
        addSigners(3);
        TimelockController timelock = hsgsuper.timelock();

        uint256 preNonce = safe.nonce();
        uint256 preValue = 5 ether;
        uint256 transferValue = 0.2 ether; // .2 ether fails guard pre-check
        address destAddress = guard.TEST_ADDRESS();
        ...
        vm.expectRevert("Guard pre-check failed.");
        ...
        transferValue = 2.2 ether; // 2.2 ether fails guard post-check

        // create the tx
        txHash = getTxHash(destAddress, transferValue, hex"00", safe);

        // have 3 signers sign it
        signatures = createNSigsForTx(txHash, 3);
        ...
        transferValue = 1.5 ether; // 1.5 ether succeeds guard checks
    ...
    // owner can change guard
    function testAuthorityCanChangeGuard() public {
        mockIsWearerCall(address(this), ownerHat, true);
        MockGuard guard = new MockGuard();
        hsgsuper.changeGuard(address(guard));
        MockGuard guard2 = new MockGuard();
        hsgsuper.changeGuard(address(guard2));
    }
    // non-owner cannot change guard
    function testNonAuthorityCannotChangeGuard() public {
        MockGuard guard = new MockGuard();
        vm.prank(addresses[0]);
        vm.expectRevert();
        hsgsuper.changeGuard(address(guard));
    }
    // owner can remove guard
    function testAuthorityCanRemoveGuard() public {
        mockIsWearerCall(address(this), ownerHat, true);
        MockGuard guard = new MockGuard();
        hsgsuper.changeGuard(address(guard));
        hsgsuper.changeGuard(address(0));
        addSigners(3);
        TimelockController timelock = hsgsuper.timelock();

        uint256 preNonce = safe.nonce();
        uint256 preValue = .5 ether; // with guard this would revert, but without should be fine
    ...
    function testAuthorityCanAddNewCanceller() public {
        mockIsWearerCall(address(this), ownerHat, true);
        addSigners(3);
        TimelockController timelock = hsgsuper.timelock();
        address newCanceller = addresses[4];

        hsgsuper.setCanceller(newCanceller);
        ...
        // confirm proposal is in the timelock
        assertEq(timelock.isOperation(id), true);
        vm.prank(newCanceller);
        timelock.cancel(id);

        assertEq(timelock.isOperation(id), false);
    }
    // test authority removes canceller
    function testAuthorityCanRemoveCanceller() public {
        mockIsWearerCall(address(this), ownerHat, true);
        addSigners(3);
        TimelockController timelock = hsgsuper.timelock();

        hsgsuper.removeCanceller(canceller);
